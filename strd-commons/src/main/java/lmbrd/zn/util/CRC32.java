...
public class CRC32 {
    ...
    public static int crc32(ByteBuffer buf) {
        return crc32(buf, 0, buf.remaining());
    }

    public static int crc32( byte[] buf, int index, int len) {
    ...
    public static int crc32( ByteBuffer buf, int index, int len) {
        final int endIndex = index + len;
        ...
        for (int i = index; i < endIndex; i ++) {
            crc32 = crc32 >>> 8 ^ TABLE[(crc32 ^ buf.get(i)) & 0xff];
        }
