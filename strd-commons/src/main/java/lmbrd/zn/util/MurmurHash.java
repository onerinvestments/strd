...
public class MurmurHash extends Hash {
	...
	public int hash(CharSequence data, int offset, int length, int seed) {
		int m = 0x5bd1e995;
		int r = 24;
		...
		for ( int i = 0; i < len_4; i++ ) {
			int i_4 = (i << 2) + offset;
			int k = data.charAt( i_4 + 3 );
			k = k << 8;
			k = k | (data.charAt( i_4 + 2 ) & 0xff);
			k = k << 8;
			k = k | (data.charAt( i_4 + 1 ) & 0xff);
			k = k << 8;
			//noinspection PointlessArithmeticExpression
			k = k | (data.charAt( i_4 + 0 ) & 0xff);
			k *= m;
			k ^= k >>> r;
			k *= m;
			h *= m;
		...
		if ( left != 0 ) {
			if ( left >= 3 ) {
				h ^= data.charAt(i_m + 2 ) << 16;
			}
			if ( left >= 2 ) {
				h ^= data.charAt( i_m + 1 ) << 8;
			}
			if ( left >= 1 ) {
				h ^= data.charAt( i_m );
			}
